...
void *naive_malloc_align(size_t size)
{
	...
	if (blk_0_addr == NULL && used_blk_ct == 0)
	{
		...
		if (page_sz == -1)
		{
			fprintf(stderr, "naive_malloc_align: sysconf failure\n");
			return (NULL);
		}
		...
		{
			perror("naive_malloc_align: sbrk");
			return (NULL);
		}
	}
	...
	/* simple alignment presumes alignment of starting progam break and previous blocks */
